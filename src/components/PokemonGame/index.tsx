...
export default function PokemonGame() {
  ...
  const handleWorldBuilderSubmit = useCallback(async (prompt: string) => {
    ...
    if (inElectron && electronAgents?.createAgent && electronFS?.projects?.length) {
      setShowWorldBuilderPrompt(false);
      const commandCenterProject = electronFS.projects.find(p => p.path.includes('claude-command-center'));
      const projectPath = commandCenterProject?.path || electronFS.projects[0]?.path;
  ...
  // Export zone as .claude-command-center-world file
  ...
  // Import .claude-command-center-world file
