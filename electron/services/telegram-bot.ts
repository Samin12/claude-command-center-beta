...
function sendUnauthorizedMessage(chatId: string | number) {
  telegramBot?.sendMessage(chatId,
    ...
    `_Get the token from Claude Command Center Settings → Telegram_`,
    { parse_mode: 'Markdown' }
  );
}
...
export function initTelegramBot() {
  ...
  try {
    ...
    telegramBot.onText(/\/auth\s+(.+)/, (msg, match) => {
      ...
      if (!appSettings.telegramAuthToken) {
        telegramBot?.sendMessage(chatId,
          ...
          '_Generate one in Claude Command Center Settings → Telegram_',
          { parse_mode: 'Markdown' }
        );
        return;
      }
      ...
      if (providedToken === appSettings.telegramAuthToken) {
        ...
        telegramBot?.sendMessage(chatId,
          '❌ *Invalid token*\n\n' +
          '_Check your token in Claude Command Center Settings → Telegram_',
          { parse_mode: 'Markdown' }
        );
      }
    });
    ...
    telegramBot.onText(/\/start$/, (msg) => {
      ...
      telegramBot?.sendMessage(chatId,
        `👑 *Claude Command Center Bot Connected!*\n\n` +
...
export async function sendToSuperAgent(chatId: string, message: string, attachedFiles?: string[]) {
  ...
  if (!superAgent) {
    telegramBot?.sendMessage(chatId,
      '👑 No Super Agent found.\n\nCreate one in Claude Command Center first, or use /start\\_agent to start a specific agent.',
      { parse_mode: 'Markdown' }
    );
    return;
  }
