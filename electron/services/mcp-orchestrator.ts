...
export async function setupMcpOrchestrator(appSettings?: AppSettings): Promise<void> {
  try {
    ...
    const mcpServers: Array<{ name: string; serverPath: string }> = [
      ...
      { name: 'claude-command-center-socialdata', serverPath: getMcpSocialDataPath() },
      { name: 'claude-command-center-x', serverPath: getMcpXPath() },
      { name: 'claude-command-center-world', serverPath: getMcpWorldPath() },
    ];
