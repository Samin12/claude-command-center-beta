      ...
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Agent "${agentName}" (${data.agent.status}): No clean output captured yet. The agent's terminal output is available in the Claude Command Center UI. Clean output is captured when the agent pauses or completes.`,
            },
          ],
        };
      } catch (error) {
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Agent "${agentName}" completed (${waitData.status}). No clean output captured — check the agent's terminal in Claude Command Center UI for details.`,
            },
          ],
        };
      } catch (error) {
